...
// where effective_load = max(0, raw_tokens - matched_tokens_from_prefix_cache).
func CachedProcessTokenSelectWorker(ctx context.Context, workers []string, message string) (string, error) {
	...
	for _, w := range workers {
		...
		if raw > matched {
			effective = raw - matched
		}

		logger.Debug(ctx, "cached_process_token: worker=%s raw=%d matched=%d effective=%d", w, raw, matched, effective)
